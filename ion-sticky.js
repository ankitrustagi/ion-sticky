            ...
            link: function ($scope, $element, $attr, $ionicScroll) {
                ...
                var updateSticky = ionic.throttle(function() {
                    ...
                    for (var i = 0; i < dividers.length; ++i) { // can be changed to binary search
                        if ($ionicPosition.offset(dividers[i]).top - dividers[i].prop('offsetHeight') < 123) { // this equals to jquery outerHeight
                            if (i === dividers.length-1 || $ionicPosition.offset(dividers[i+1]).top -
                                 (dividers[i].prop('offsetHeight') + dividers[i+1].prop('offsetHeight')) > 123) {
                                active = dividers[i][0];
                                break;
                            }
                        }
